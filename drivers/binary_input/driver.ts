...
module.exports = class BinaryInputDriver extends BaseDriver {
  ...
  private formatForPairing(devices: XComfortDevice[]) {
    ...
    const candidates = devices
      .filter((device) => {
        ...
        if (!isGenericBinaryInputDevice(device, component)) {
          return false;
        }
      ...
      .map((device) => {
        ...
        const component = device.compId !== undefined ? bridge.getComponent(String(device.compId)) : undefined;
        return {
          name: this.getDisplayNameWithBridge(getDisplayName(device, 'Binary Input'), device),
          data: {
            ...this.getBridgeDeviceData('binary_input', device),
