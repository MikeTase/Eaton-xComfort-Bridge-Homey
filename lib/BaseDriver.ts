...
interface XComfortApp extends Homey.App {
    ...
    updateBridgeConfig?: (bridgeId: string | null, ip: string, authKey: string) => Promise<void>;
    waitForBridgeConnection?: (bridgeId: string | null, timeoutMs?: number) => Promise<boolean>;
}
...
export abstract class BaseDriver extends Homey.Driver {
    ...
    /**
     * Repair flow: lets the user update the IP address and auth key of the
     * bridge this device belongs to, without removing and re-pairing the
     * device (which would break Flows).
     */
    onRepair(session: Homey.PairSession, device: Homey.Device): void {
        session.setHandler('login', async (data: { username?: string; password?: string }) => {
            const app = this.homey.app as unknown as XComfortApp;

            if (typeof app.updateBridgeConfig !== 'function' || typeof app.waitForBridgeConnection !== 'function') {
                throw new Error('Repair is not supported by this app version.');
            }

            const deviceData = device.getData() as { bridgeId?: string } | null;
            const bridgeId = deviceData?.bridgeId || null;

            await app.updateBridgeConfig(bridgeId, data.username ?? '', data.password ?? '');

            const connected = await app.waitForBridgeConnection(bridgeId, 25000);
            if (!connected) {
                throw new Error('Could not connect to the bridge with these settings. Check the IP address and authentication key, then try again.');
            }
    ...
    private async getDevicesForBridge(bridge: XComfortBridge, timeoutMs: number): Promise<XComfortDevice[]> {
        ...
        return new Promise<XComfortDevice[]>((resolve) => {
            ...
            timerRef.current = setTimeout(() => finish(bridge.getDevices() || []), timeoutMs);
        });
    }
    ...
    private async getRoomsForBridge(bridge: XComfortBridge, timeoutMs: number): Promise<XComfortRoom[]> {
        ...
        return new Promise<XComfortRoom[]>((resolve) => {
            ...
            bridge.once('devices_loaded', onLoaded);
            timerRef.current = setTimeout(finish, timeoutMs);
        });
    }
