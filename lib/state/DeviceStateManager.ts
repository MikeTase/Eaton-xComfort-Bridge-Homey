...
export class DeviceStateManager {
  ...
  deleteDevice(deviceId: string | number): void {
    this.devices.delete(String(deviceId));
  }
  ...
  deleteComponent(compId: string | number): void {
    this.components.delete(String(compId));
  }
  ...
  addListener(deviceId: string | number, callback: DeviceStateCallback): void {
    ...
    if (deviceListeners.includes(callback)) {
      return;
    }
  ...
  addRoomListener(roomId: string | number, callback: RoomStateCallback): void {
    ...
    if (roomListeners.includes(callback)) {
      return;
    }
