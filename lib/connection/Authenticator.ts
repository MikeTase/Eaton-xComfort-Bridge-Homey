...
export class Authenticator {
  ...
  constructor(
    ...
    authOptions?: XComfortAuthOptions
  ) {
  ...
  handleEncryptedMessage(msg: ProtocolMessage): boolean {
    if (msg.type_int === MESSAGE_TYPES.SECRET_EXCHANGE_ACK) {
      ...
      this.logger('[Authenticator] Sent login request');
      return true;
    }

    if (msg.type_int === MESSAGE_TYPES.LOGIN_DENIED) {
      this.logger('[Authenticator-ERROR] Login DENIED by bridge - check credentials in Settings');
      this.state = 'failed';
      return true;
    }
